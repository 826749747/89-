...
typedef enum  {     NUL, IDENT, NUMBER, PLUS, MINUS, TIMES,
	            ...
	            CONSTSYM, VARSYM, PROCSYM, PROGSYM,
                    ELSESYM, FORSYM,STEPSYM,UNTILSYM,RETURNSYM,    //添加单词
                    TIMESBECOMES,SLASHBECOMES,ADNSYM,ORSYM,NOTSYM,    //添加运算符
                    INCSYM,DECSYM                                   //添加++ --
...
char *SYMOUT[] = {"NUL", "IDENT", "NUMBER", "PLUS", "MINUS", "TIMES",
	    ...
	    "WHILESYM", "WRITESYM", "READSYM", "DOSYM", "CALLSYM",
	    "CONSTSYM", "VARSYM", "PROCSYM", "PROGSYM",
            "ELSESYM","FORSYM","STEPSYM","UNTILSYM","DOSYM","RETURNSYM",   //单词
            "TIMESBECOMES","SLASHBECOMES","ADNSYM","ORSYM","NOTSYM"          //运算符
             };
...
SYMSET SymSetUnion(SYMSET S1, SYMSET S2) {
  SYMSET S=(SYMSET)malloc(sizeof(int)*NUMSYM);
...
SYMSET SymSetAdd(SYMBOL SY, SYMSET S) {
  SYMSET S1;
  S1=(SYMSET)malloc(sizeof(int)*NUMSYM);
  for (int i=0; i<NUMSYM; i++) S1[i]=S[i];
  S1[SY]=1;
  return S1;
}
//---------------------------------------------------------------------------
SYMSET SymSetNew(SYMBOL a) {
  SYMSET S; int i,k;
  S=(SYMSET)malloc(sizeof(int)*NUMSYM);
  for (i=0; i<NUMSYM; i++) S[i]=0;
  S[a]=1;
  return S;
}
//---------------------------------------------------------------------------
...
void STATEMENT(SYMSET FSYS,int LEV,int &TX) {   /*STATEMENT*/
  ...
  case IDENT:
		...
		if (SYM==BECOMES||SYM==TIMESBECOMES||SYM==SLASHBECOMES)
                {
                  OPSYM=SYM;
                  if(SYM==TIMESBECOMES||SYM==SLASHBECOMES)
                  {
                   GEN(LOD,LEV-TABLE[i].vp.LEVEL,TABLE[i].vp.ADR);//取值到栈顶
                  }
                  GetSym();
                  EXPRESSION(FSYS,LEV,TX);
                  if(OPSYM==TIMESBECOMES) GEN(OPR,0,4);//次栈顶与栈顶相乘
                  if(OPSYM==SLASHBECOMES) GEN(OPR,0,5);//次栈顶除以栈顶
                ...
                else
                {
                 ...
                 else
                 {
				  ...
				  else
				  {
					  GetSym();
					  EXPRESSION(SymSetUnion(SymSetNew(UNTILSYM, STEPSYM, DOSYM), FSYS), LEV, TX);
					  GEN(LOD,LEV-TABLE[i].vp.LEVEL,TABLE[i].vp.ADR);//将循环变量放入栈顶
					  GEN(OPR, 0, 2);
					  GEN(STO, LEV - TABLE[i].vp.LEVEL, TABLE[i].vp.ADR);//栈顶值放入循环变量
					  ...
					  {
						  CODE[CX1].A = CX;//如果是赋值语句，跳转到UntilSym
						  GetSym();
						  EXPRESSION(SymSetUnion(SymSetNew(UNTILSYM, STEPSYM, DOSYM), FSYS), LEV, TX);
						  GEN(LOD, LEV - TABLE[i].vp.LEVEL, TABLE[i].vp.ADR);
						  GEN(OPR,0,12);//次栈顶是否比栈顶小
						  CX3 = CX;
						  GEN(JPC, 0, 0);//栈顶非0则跳转
...
void __fastcall TForm1::ButtonRunClick(TObject *Sender) {
  ...
  DECLBEGSYS=(int*)malloc(sizeof(int)*NUMSYM);
  STATBEGSYS=(int*)malloc(sizeof(int)*NUMSYM);
  FACBEGSYS =(int*)malloc(sizeof(int)*NUMSYM);
